...
class App {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    ...
    // Concatenate twice so circular scrolling loops seamlessly
  ...
  onTouchUp(e) {
    ...
    // Click detection: if user tapped an item without dragging, trigger onItemClick
    if (!this.hasMoved && this.onItemClick && this.medias && this.medias.length) {
      ...
      for (let i = 0; i < this.medias.length; i++) {
        ...
        const screenCenterX = this.screen.width / 2 + (media.plane.position.x / (this.viewport.width / 2)) * (this.screen.width / 2);
        const dist = Math.abs(relX - screenCenterX);
        if (dist < minDistance) {
          minDistance = dist;
          closest = this.mediasImages[i];
        }
      }
    ...
    switch (e.key) {
      ...
      case 'Enter':
        if (this.onItemClick && this.medias && this.medias[0]) {
          ...
          this.onItemClick(this.mediasImages[itemIndex]);
        }
        break;
...
export default function CircularGallery({
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="region"
      aria-label="Circular image gallery. Drag or scroll to rotate categories. Press Enter or click to select."
    />
  );
}
